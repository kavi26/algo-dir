...
class minHeap{
    private:
     ...
     int getLabel(int a) { return label[a]; }
     ...
     void Heapify(int a){ // Heapify from node a
        ...
        while(i < Size){
         ...
         int r = Right(i);
         if(l <= Size && r <= Size){
              if(label[H[l]] > label[H[r]] && label[H[i]] > label[H[r]]){ // right child is minimum and smaller than parent
               swap(H[r], H[i]);
               i = r;
               continue;        
              }
              else if(label[H[l]] < label[H[r]] && label[H[i]] > label[H[l]]){ // left child is minimum and smaller than parent
               swap(H[l], H[i]);
               i = l;
               continue;
              }
         ...
         else if(l <= Size && label[H[i]] > label[H[l]]){
              swap(H[l], H[i]);
              i = l;
              continue;
         } else if(r <= Size && label[H[i]] > label[H[r]]){
              swap(H[r], H[i]);
              i = r;
              continue;
         } else break;
